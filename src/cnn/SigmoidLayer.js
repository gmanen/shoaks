...
class SigmoidLayer extends Layer {
        ...
        for (let d = 0; d < this.output.depth; d++) {
            for (let h = 0; h < this.output.height; h++) {
                for (let w = 0; w < this.output.width; w++) {
                    this.output.data[d][h][w] = 1 / (1 + Math.exp(-input.data[d][h][w]))
                }
            }
        }
    ...
    backPropagate() {
        for (let d = 0; d < this.output.depth; d++) {
            for (let h = 0; h < this.output.height; h++) {
                for (let w = 0; w < this.output.width; w++) {
                    this.input.gradients[d][h][w] = this.output.data[d][h][w] * (1 - this.output.data[d][h][w]) * this.output.gradients[d][h][w]
                }
            }
        }
    }
